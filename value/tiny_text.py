...
SIZE_MAX = 255
SIZE_OF_CONTENT_SIZE = 1


class TinyText(Value):

    def __init__(self, content: str) -> None:
        if len(content) > SIZE_MAX:
            raise ValueError("字符串长度超出限制")
        super().__init__(ValueType.TINY_TEXT, content)
...
def new_tiny_text(content: str) -> TinyText:
    return TinyText(content)


def new_tiny_text_from_bytes(buf: BytesIO) -> TinyText:
